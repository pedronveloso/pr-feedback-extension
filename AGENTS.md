...
## Testing Guidelines
Vitest runs in a `jsdom` environment, so DOM-facing code should be covered with focused unit tests. Add or update fixture-based integration tests when GitHub DOM parsing changes. Keep fixtures under `tests/fixtures/` and name tests by behavior, not implementation. Before opening a PR, run `npm run typecheck` and `npm test`; CI runs both on every pull request.
